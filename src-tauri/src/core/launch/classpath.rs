...
pub fn build_classpath(
    ...
    let mut best_asm: HashMap<String, (String, String)> = HashMap::new();
    ...
    for raw in extra_lib_coords {
        ...
        if let Ok(artifact) = MavenArtifact::parse(trimmed) {
            // Handle ASM conflicts (critical for Forge)
            if artifact.group_id == "org.ow2.asm" {
                let key = format!(
                    "{}:{}",
                    artifact.artifact_id,
                    artifact.classifier.clone().unwrap_or_default()
                );

                match best_asm.get(&key) {
                    None => {
                        best_asm.insert(key, (trimmed.to_string(), artifact.version));
                    }
                    Some((_, existing)) => {
                        if compare_versions(&artifact.version, existing) == Ordering::Greater {
                            best_asm.insert(key, (trimmed.to_string(), artifact.version));
                        }
                    }
                }
        ...
        if let Some(p) = resolve_library_entry(instance, libs_dir, trimmed) {
            other_libs.push(p);
        }
    }

    // Newest ASM first
    let mut asm: Vec<_> = best_asm.into_values().collect();
    ...
    for jar in collect_local_library_jars(instance) {
